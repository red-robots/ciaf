...
/* Query Venues with active events */
function get_venues_events_list() {
  global $wpdb;

  /* Query Events with assigned Venue */
  $query_events = "SELECT p.ID,p.post_title,m.meta_value FROM ".$wpdb->prefix."posts p,".$wpdb->prefix."postmeta m WHERE p.ID=m.post_id AND p.post_type='tribe_events' AND p.post_status='publish' AND m.meta_key='_EventVenueID' AND m.meta_value!=''";
  $events_result = $wpdb->get_results($query_events);
  
  //echo "<pre>";
  $venueList = array();
  if($events_result) {
    foreach($events_result as $e) {
      $post_id = $e->ID;
      $meta_value = $e->meta_value;

      /* Query Venue by post ID */
      $query_venues = "SELECT p.ID,p.post_title,p.post_type FROM ".$wpdb->prefix."posts p  WHERE p.ID=".$meta_value." AND p.post_type='tribe_venue' AND p.post_status='publish' ORDER BY p.post_title ASC";
      $venues_result = $wpdb->get_results($query_venues);
      $eventsList = array();
      $total = 0;
      if($venues_result) {
        $venueGroup = [];
        foreach($venues_result as $v) {
          $v->VenueLat = get_post_meta($v->ID,'_VenueLat',true);
          $v->VenueLng = get_post_meta($v->ID,'_VenueLng',true);
          $venueList[$v->ID] = $v;
        }
      }
...
global $a, $baseUrl;
$baseUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['SERVER_NAME'] . parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
$a = shortcode_atts( array(
  'center' => '35.22753598160502, -80.84154190157783',
  'zoom' => 10,
  'perpage'=>10
), $atts );
$a['venue_id'] = ( isset($_GET['venueid']) && $_GET['venueid'] ) ? $_GET['venueid'] : '';

$center = ( isset($a['center']) && $a['center'] ) ? explode(",",$a['center']) : '';
  if($center) {
    $eventList = '';
    ...
    $venue_id = (isset($a['venue_id']) && $a['venue_id']) ? $a['venue_id'] : '';
    ob_start();
    include( locate_template('parts/events-by-venue.php') );
    $eventList = ob_get_contents();
    ob_end_clean();
    ob_end_flush();
    $eventList = ($eventList) ? preg_replace('#^<\/p>|<p>$#', '', $eventList) : ''; 
    ...
    add_action('wp_footer', function () { 
      global $a,$baseUrl;
      $center = ( isset($a['center']) && $a['center'] ) ? explode(",",$a['center']) : '';
      $centerLat = (isset($center[0]) && $center[0]) ? $center[0] : '';
      $centerLon = (isset($center[1]) && $center[1]) ? $center[1] : '';
      $zoom_val = (isset($a['zoom']) && $a['zoom']) ? $a['zoom'] : 5;
    ?>
    <style>
    .venue-map-wrapper {position:relative;}
    #tribe-venue-map {
      width: 100%; height: 770px; min-height: 400px;
    }
    #tribe-venue-map > div {
      height: 100%;
    }
    ...
    <script>
      document.body.classList.add("has-tribe-venue-map");
      function tribe_venue_map() {
        var map = new google.maps.Map(document.getElementById('tribe-venue-map'), {
        center: {lat: <?php echo $centerLat ?>, lng: <?php echo $centerLon ?>},
        zoom: <?php echo $zoom_val ?>
        });

        /* Turn Off some landmarks / features => https://mapstyle.withgoogle.com/ */
        ...
        <?php $venues = get_posts( array( 'post_type' => Tribe__Events__Main::VENUE_POST_TYPE, 'posts_per_page' => -1) );
        foreach ( $venues as $venue ) {
          if( $venue->post_title ) {
            ...
            if ( $coordinates['lat'] != 0 && $coordinates['lng'] != 0 ) { ?>
              ...
              content: '<div class="map-venue-name" data-venue-id="<?php echo $venue->ID; ?>"><a href="javascript:void(0)" class="map-venue-info" data-url="<?php echo $baseUrl ?>?venueid=<?php echo $venue->ID?>"><?php echo $venue->post_title; ?></a></div>'
              });
    ...
    jQuery(document).ready(function($){
      tribeEventAction();
      $(document).on('click','#tribe-events-pagination a',function(e){
        e.preventDefault();
        var pageLink = $(this).attr('href');
        loadEventsByVenueList(pageLink);
      });

      $(document).on('click','a.map-venue-info',function(e){
        e.preventDefault();
      ...
      /* Close Map Bubble */
      $(document).on('click','#tribe-venue-map div[role="button"] map',function(e){
        e.preventDefault();
        var target = $(this).find('button[aria-label="Close"]');
        //$('#tribe-venue-map button[aria-label="Close"]').not(target).trigger('click');
      });

      function tribeEventAction() {
        $('.tribe-events-list p:empty').remove();
        $('.tribe-events-list .address p').contents().unwrap();
        $('#tribe-events-pagination br').remove();
      }

      function loadEventsByVenueList(pageLink) {
        $('#events-loader').show();
        $('#eventsContainer').load(pageLink+' #eventsByVenue',function(){
          tribeEventAction();
          if( $('#eventsContainer .norecord').length ) {
            $('#venue-events-container').addClass('norecord');
          } else {
            $('#venue-events-container').removeClass('norecord');
          }
          setTimeout(function(){
            $('#events-loader').hide();
          },500);
    ...
    <?php });
    return '<div class="venue-map-wrapper"><div id="tribe-venue-map"></div><div id="venue-events-container"><div id="eventsContainer">'.$eventList.'</div><div id="events-loader" class="lds-roller"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div></div>';
  }
}
